...
void Simple2DSonar::loadEnvironment(ros::NodeHandle& nh) {
    ...
    if (nh.getParam("/env_spawner_node/obstacles", obs_list)) {
        if (obs_list.getType() == XmlRpc::XmlRpcValue::TypeArray) {
            ...
            ROS_INFO("[Simple2DSonar] Successfully loaded %zu obstacles from ROS Parameter Server.", obstacles_.size());
        }
    } else {
        ROS_WARN("[Simple2DSonar] Failed to get obstacles from parameter server! Ensure env_spawner is running.");
    }
}
...
void Simple2DSonar::generateData(const uuv_interface::SensorState& state) {
    ...
    if (!frame_id.empty() && frame_id[0] == '/') {
        frame_id = frame_id.substr(1); // 截掉第一个字符 '/'
    }
    scan.header.frame_id = frame_id + "/base_link";
...
double Simple2DSonar::checkIntersection(const Eigen::Vector3d& origin, const Eigen::Vector3d& dir) {
    double min_dist = 1e9;
    ...
    for (const auto& obs : obstacles_) {
        if (obs.type == "sphere") {
            ...
            Eigen::Vector3d L = obs.pos - origin;
            ...
            Eigen::Vector2d O2d(origin.x(), origin.y());
            Eigen::Vector2d D2d(dir.x(), dir.y());
            ...
            if (discriminant >= 0) {
                ...
                if (t0 > 0) {
                    double hit_z = origin.z() + t0 * dir.z();
            ...
            Eigen::Vector3d inv_dir(1.0 / dir.x(), 1.0 / dir.y(), 1.0 / dir.z());

            double tx1 = (min_bound.x() - origin.x()) * inv_dir.x();
            double tx2 = (max_bound.x() - origin.x()) * inv_dir.x();
            double tmin = std::min(tx1, tx2);
            double tmax = std::max(tx1, tx2);

            double ty1 = (min_bound.y() - origin.y()) * inv_dir.y();
            double ty2 = (max_bound.y() - origin.y()) * inv_dir.y();
            tmin = std::max(tmin, std::min(ty1, ty2));
            tmax = std::min(tmax, std::max(ty1, ty2));

            double tz1 = (min_bound.z() - origin.z()) * inv_dir.z();
            double tz2 = (max_bound.z() - origin.z()) * inv_dir.z();
