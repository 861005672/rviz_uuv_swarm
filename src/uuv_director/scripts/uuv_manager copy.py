...
class UUVManager:
    def __init__(self):
        ...
        self.passive_sonar_delay = rospy.get_param('~passive_sonar_delay', 0.4) 
        self.active_sonar_delay = rospy.get_param('~active_sonar_delay', 1.0)   
        self.noise_dist_std = rospy.get_param('~noise_dist_std', 2.0)           
        self.noise_ang_std = rospy.get_param('~noise_ang_std', 0.05)            
        self.miss_prob = rospy.get_param('~miss_prob', 0.05)                    
        self.false_prob = rospy.get_param('~false_prob', 0.02)                  
    ...
    def _build_neighbor_msg(self, self_id, neighbor_id, real_dist, real_self_state, real_nb_state, t_passive, t_active):
        ...
        _, _, _, p_ryaw, p_rpitch = self.get_relative_info(p_self_state, p_dx, p_dy, p_dz)
        
        # =========================================================
        # 核心修改：突变（野值） vs 正常高斯噪声
        # =========================================================
        if random.random() < self.false_prob:
            # 【发生突变】：模拟声纳受到剧烈干扰，数据瞬间乱跳
            msg.passive_distance = random.uniform(2.0, self.nb_sense_radius) # 距离在探测范围内随机乱飞
            msg.passive_direction_azimuth = random.uniform(-math.pi, math.pi) # 方位角全向乱飞
            msg.passive_direction_elevation = random.uniform(-math.pi/4, math.pi/4)
        else:
            # 【正常探测】：仅带有正态分布的高斯噪声
            msg.passive_distance = max(0.5, p_dist + random.gauss(0, self.noise_dist_std))
            msg.passive_direction_azimuth = p_ryaw + random.gauss(0, self.noise_ang_std)
            msg.passive_direction_elevation = p_rpitch + random.gauss(0, self.noise_ang_std)
        # =========================================================

        msg.passive_delay_time = self.passive_sonar_delay
        msg.passive_distance_variance = self.noise_dist_std ** 2
        msg.passive_direction_variance = self.noise_ang_std ** 2
